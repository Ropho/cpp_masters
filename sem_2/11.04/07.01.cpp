...
public:
	...
	auto operator()(std::monostate) const -> void
	{
		std::cout << "infinite roots" << std::endl;
	}

	auto operator()(double root) const -> void
	{
		std::cout << "one root: x = " << root << std::endl;
	}

	auto operator()(std::pair < double, double > roots) const -> void
	{
		auto [x1, x2] = roots;
